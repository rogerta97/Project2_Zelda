...
bool Player::Update(float dt)
{
	...
	if (App->input->GetKey(SDL_SCANCODE_A) == KEY_REPEAT || App->input->GetControllerButton(0, SDL_CONTROLLER_BUTTON_DPAD_LEFT) == KEY_REPEAT)
	{
		player_go->SetPos({ player_go->fGetPos().x - speed, player_go->fGetPos().y });
		flip = true;
	}
	else if (App->input->GetKey(SDL_SCANCODE_D) == KEY_REPEAT || App->input->GetControllerButton(0, SDL_CONTROLLER_BUTTON_DPAD_RIGHT) == KEY_REPEAT)
	{
		player_go->SetPos({ player_go->fGetPos().x + speed, player_go->fGetPos().y });
		flip = false;
	}
	if ((App->input->GetKey(SDL_SCANCODE_W) == KEY_DOWN || App->input->GetControllerButton(0, SDL_CONTROLLER_BUTTON_A) == KEY_DOWN) && on_ground)
	{
...
bool Player::Draw(float dt)
{
	...
	if (on_ground)
	{
		...
		if (App->input->GetKey(SDL_SCANCODE_W) == KEY_DOWN || App->input->GetControllerButton(0, SDL_CONTROLLER_BUTTON_A) == KEY_DOWN)
			on_ground = false;
	}
